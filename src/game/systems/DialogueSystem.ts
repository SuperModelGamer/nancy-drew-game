...
const EVENT_JOURNAL_ENTRIES: Record<string, string> = {
  ...
  answered_parents_call: 'Dad called. He warned me to be careful — if someone here is willing to use poison, they\'re dangerous. He also said Vivian\'s family has a complicated history with this theater.',
  ...
  vivian_sent_upstairs: 'Vivian told me to head upstairs. Edwin Hale is in the auditorium, and Stella and Diego are somewhere in the building. She warned me: "This theater has a way of showing you things you weren\'t looking for."',
  edwin_auditorium: 'Met Edwin Hale — the self-appointed theater historian. He\'s intense, protective of the Monarch, and knows more about this building than anyone alive. He treats every scratch on the armrests like a historical artifact.',
  stella_backstage: 'Met Stella Morrow, the stage manager. She\'s sharp, stressed, and clearly not happy about the demolition. She knows every inch of this building — and she\'s noticed things that don\'t add up.',
...
const EVENT_THINKING_HINTS: Record<string, string> = {
  ...
  answered_parents_call: 'Thinking: Dad\'s right — poison means someone dangerous is in this building. I need to be careful, but I also need to keep moving. The lobby has more to tell me.',
  ...
  vivian_sent_upstairs: 'Thinking: Vivian says Edwin knows what Ashworth is really after. And Stella and Diego have been "restless." I should find all three.',
  edwin_auditorium: 'Thinking: Edwin is passionate about this theater — maybe too passionate. He knows about hidden rooms, secret passages, and the 1928 death. I should ask him about The Crimson Veil.',
  stella_backstage: 'Thinking: Stella runs this place. If something\'s happening behind the scenes — literally — she\'d know about it. I should ask about the props and the ghost sightings.',
  ...
  ashworth_snooping: 'Thinking: Ashworth stepped out to take a call. His laptop is open, his desk is unguarded. I need to search quickly before he comes back.',
  edwin_basement_lie: 'Thinking: Edwin says he hasn\'t been in the basement in years. But Stella says she sees him going down there at 2 AM. One of them is lying. My money is on Edwin.',
  overheard_stella: 'Thinking: Stella is covering for Edwin. "She doesn\'t know about Edwin. Not yet." She knows about the ghost staging and she\'s been keeping it quiet. Time to confront her.',
  stella_phone_confession: 'Thinking: Stella confessed — she\'s known about Edwin\'s ghost staging for weeks. She was trying to protect the landmark petition. But she doesn\'t think he poisoned Ashworth.',
  called_bess_day2: 'Thinking: Ashworth\'s company has filed three insurance claims in five years — all on demolished properties. A pattern of manufactured hazards. The poisoning might not be what it seems.',
  called_george_day2: 'Thinking: The building inspection was buried. The inspector — M. Lawson — signed off on Ashworth\'s previous demolitions too. And the sealed report says the theater is structurally sound. Someone is fabricating a reason to tear it down.',
  called_ned_day2: 'Thinking: Ned says the non-lethal dose is actually harder to calculate than a lethal one. Whoever poisoned Ashworth knew exactly how much to use — and the easiest way to know is to do it to yourself.',
  diego_receipt_match: 'Thinking: Diego found the connection — the coat check receipt and Ashworth\'s trash both trace to the same shell company, Monarch Properties LLC. Ashworth ordered the poison that was used on him. The "victim" is the culprit.',
  found_warning_note: 'Thinking: Someone left a threat on the stage. "Stop digging or the next fall won\'t be an accident." I\'m getting closer — and someone is scared.',
  edwin_denies_poison: 'Thinking: Edwin\'s chemicals are crude and homemade. The poison in Ashworth\'s tea was pharmaceutical grade. Edwin staged the ghost — but I don\'t think he poisoned anyone.',
  edwin_confronted: 'Thinking: Edwin confessed to the ghost staging. He did it to save the theater and honor his grandfather\'s promise. But the poisoning is a separate crime — and the evidence points somewhere Edwin would never look.',
...
export class DialogueSystem {
    ...
    if (line.vo) {
      ...
      if (this.scene.cache.audio.exists(line.vo)) {
        try {
          ...
          this.currentVO.play();
          MusicSystem.getInstance().duck();
          this.currentVO.once('complete', () => MusicSystem.getInstance().unduck());
  ...
  private stopVO(): void {
    if (this.currentVO) {
      ...
      this.currentVO = null;
      MusicSystem.getInstance().unduck();
    }
  }
