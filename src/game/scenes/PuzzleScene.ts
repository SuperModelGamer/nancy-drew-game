...
export class PuzzleScene extends Phaser.Scene {
  ...
  create(): void {
    ...
    const panelH = Math.min(height * 0.75, 750);
    ...
    // Hint text
    this.hintText = this.add.text(0, panelH / 2 - 83, '', {
      fontFamily: FONT,
      fontSize: '20px',
    ...
    // Close button
    const closeBtn = createCloseButton(this, panelW / 2 - 45, -panelH / 2 + 33, () => this.closePuzzle(), 99);
  ...
  private buildCombinationUI(panelW: number, _panelH: number, answer: string): void {
    ...
    for (let i = 0; i < this.dialCount; i++) {
      ...
      // Dial display
      const dialBg = this.add.rectangle(x, dialY, 75, 83, Colors.sceneBg, 0.9);
  ...
  private buildSequenceUI(panelW: number, _panelH: number, answer: string): void {
    ...
    // Sequence display
    this.sequenceDisplay = this.add.text(0, 30, 'Sequence: (none)', {
      fontFamily: FONT,
      fontSize: '24px',
      color: TextColors.light,
    }).setOrigin(0.5);
    ...
    uniqueOptions.forEach((option, i) => {
      ...
      const btnBg = this.add.rectangle(0, 0, btnSpacing - 15, 72, color, 0.7);
